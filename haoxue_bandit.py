...
Main code for the pupilexplore experiment, defines all the necessary functions. 
Heavily rely on Psychopy library, in particular on its iohub module that we use for communicating with the eye tracker.
The old code was based on python2 and an older version of psychopy. The goal of the code here is to make it compatible with python3, psychopy3, and be able to run it on the lab computer.
...
from haoxue_arm import * # outsource arm info
...
# haoxue: we do not need getArmPosition
...
class ExperimentRuntime(EyeLink):
    ...
    Haoxue: we would like to use it for EyeLink (pylink library) instead of 
    """
    ...
    def runTrackerCalibration(self, tracker, kb, intrial = False):
        ...
        Haoxue: should check the new SR research eye tracker to see whether there is anything already implemented there to update the script
        """
