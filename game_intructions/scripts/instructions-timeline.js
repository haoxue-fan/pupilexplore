...
/**
 * Using the save_data.php file, creates, names, and writes data to a file as
 * specified by params
 *
...
$(document).ready(function() {
  ...
  const prompt_subject_id = {
    type: "survey-text",
    questions: [
      {
        prompt: "Please enter the participant's ID:",
        placeholder: "------",
        required: true,
        name: "id"
      }
    ...
    on_finish: function() {
      subject_id = jsPsych.data.getLastTrialData().values()[0].response.id;
    }
  };
  ...
  const instruction_pages = [
    ...
    sprintf($("#instructions-4").html(), stim_info.imgs[1]),
    ...
    questions: [
      {
        ...
        options: [
          ...
          "I press the '1' or '2' keys on my keyboard to select the left or \
          right slot machine, respectively.",
          "I press the left or right arrow keys keys on my keyboard to select \
          the left or right slot machine, respectively.",
          "I press the 'Q' or 'P' keys on my keyboard to select the left or \
          right slot machine, respectively."
        ],
        expected_indx: 2,
        name: "choice"
      ...
      },
      {
        prompt:
          "Which of the following statements about receiving a bonus is \
              correct?",
        options: [
          "My bonus is proportional to the total number of coins I \
        ...
        function() {
          let prompt =
            ...
            comprehension_check_info.questions.length
              ? "Great job, you passed! Now you are ready to start your \
              practice round."
  ...
  const save_data = {
    ...
    func: function() {
      if (subject_id != "debug") {
        save_data_csv(subject_id, jsPsych.data.get().csv());
      }
    },
    timing_post_trial: 0
  };
